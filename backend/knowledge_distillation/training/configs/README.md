...
| Field | Meaning |
|---|---|
| `model_name` | Hugging Face Hub id (or local path) of the base model to fine-tune. **If the checkpoint is multimodal, only its text tower is loaded.** `google/gemma-3-4b-it` carries a SigLIP vision encoder, and loading it as-is builds `Gemma3ForConditionalGeneration`, which (a) allocates 453M vision parameters this text-only project never uses and (b) runs its own loss instead of the shared `ForCausalLMLoss`, making a *third* full-size copy of the logits — `seq × 262208 × 4` bytes, ~3.75 GB on a typical example, which is precisely the allocation that OOM'd a 16GB T4. `trainer/model_loader.py`'s `resolve_text_only_config()` detects this generically (`config.get_text_config()` returns a different object only for composite configs) and narrows the load to `Gemma3ForCausalLM`, whose parameter names map onto the checkpoint's `language_model.*` tensors with nothing missing or left over. Qwen2.5 (delivery) is already text-only and takes the unchanged path. |
